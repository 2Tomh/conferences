...
export class ApiService {
  ...
  // ==========================================
  // ⭐ חדש: ATTACHMENT (PDF) ENDPOINTS
  // ==========================================

  // מעלה/מחליף את קובץ ה-PDF של הכנס, יחד עם התיאור המילולי שלו.
  // נשלח כ-multipart/form-data (לא JSON), כי מדובר בקובץ בינארי.
  ...
  // מוחק את קובץ ה-PDF המצורף לכנס (וה-metadata שלו)
  deleteAttachment(id: string): Observable<any> {
    return this.http.delete(`${this.apiUrl}/Surveys/${id}/attachment`);
  }

  // בונה את כתובת ההורדה הישירה של קובץ ה-PDF, לשימוש בקישורים/פופאפים
