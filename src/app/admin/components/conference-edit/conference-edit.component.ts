...
// // //   // ⭐ חדש: רשימת שפות זמינות לבחירה בטופס
...
// // //       date: [''],
// // //       location: [''],
// // //       // ⭐ חדש: שפת הכנס, ברירת מחדל English
// // //       language: ['English'],
// // //       audience: [''],
// // //       abstractDeadline: [''],
// // //       registrationDeadline: [''],
...
// // //       date: data.Date || data.date,
// // //       location: data.Location || data.location,
// // //       // ⭐ חדש: טוען שפה קיימת, נופל חזרה ל-English אם אין נתון
...
// // //       Name: formValue.conference,
// // //       Type: formValue.type,
// // //       Category: this.selectedCategories[0] || formValue.category || '',
// // //       Categories: this.selectedCategories,
// // //       Tagline: formValue.tagline,
// // //       Description: formValue.description,
// // //       Date: formValue.date,
// // //       Location: formValue.location,
// // //       // ⭐ חדש: שולח את השפה הנבחרת לשרת
...
// //   // רשימת שפות זמינות לבחירה בטופס
// //   availableLanguages = ['English', 'Hebrew'];
...
// //       date: data.Date || data.date,
// //       location: data.Location || data.location,
// //       // טוען שפה קיימת, נופל חזרה ל-English אם אין נתון
// //       language: data.Language || data.language || 'English',
// //       audience: data.Audience || data.audience,
// //       abstractDeadline: data.AbstractDeadline || data.abstractDeadline,
// //       registrationDeadline: data.RegistrationDeadline || data.registrationDeadline,
...
// //   // ⭐ חדש: מסיר בלוק תוכנית ספציפי לפי אינדקס
// //   removeProgramBlock(i: number): void {
// //     this.program.removeAt(i);
// //   }
...
//   // מסיר בלוק תוכנית ספציפי לפי אינדקס
//   removeProgramBlock(i: number): void {
//     this.program.removeAt(i);
//   }

//   // ⭐ חדש: מסיר מארגן ספציפי לפי אינדקס
//   removeOrganizer(i: number): void {
//     this.organizers.removeAt(i);
//   }
...
export class ConferenceEditComponent implements OnInit {
  ...
  // ⭐ חדש: מצב הקובץ המצורף (PDF)
  selectedFile: File | null = null;
  existingAttachmentFileName: string | null = null;
  ...
  constructor(
    ...
    this.conferenceForm = this.fb.group({
      ...
      // ⭐ חדש: התיאור המילולי הקצר שיוצג לצד קישור ההורדה של ה-PDF
  ...
  private populateForm(data: any): void {
    ...
    // ⭐ חדש: שם הקובץ המצורף הקיים (אם קיים), להצגה בטופס
    this.existingAttachmentFileName = data.AttachmentFileName || data.attachmentFileName || null;
    ...
    this.conferenceForm.patchValue({
      ...
      posterGuidelines: data.PosterGuidelines || data.posterGuidelines || '',
      links: data.Links || data.links || { survey: '', website: '' },
      // ⭐ חדש: התיאור המילולי הקיים של הקובץ המצורף
  ...
  // מסיר מארגן ספציפי לפי אינדקס
  removeOrganizer(i: number): void {
    this.organizers.removeAt(i);
  }

  // ⭐ חדש: נקרא כאשר המשתמש בוחר קובץ מה-input type="file"
  onFileSelected(event: Event): void {
    const input = event.target as HTMLInputElement;
    if (input.files && input.files.length > 0) {
      const file = input.files[0];
      if (file.type !== 'application/pdf') {
        alert('ניתן להעלות קבצי PDF בלבד');
        input.value = '';
  ...
  // ⭐ חדש: מבטל את בחירת הקובץ החדש שעדיין לא נשלח לשרת (לפני לחיצה על "שמור").
  // לא נוגע בשרת בכלל - רק מנקה את הבחירה המקומית.
  ...
  // מוחק את הקובץ המצורף הקיים (קריאה מיידית לשרת, לא ממתין ל"שמור")
  removeAttachment(): void {
    const id = this.conferenceForm.get('id')?.value;
    if (!id) return;
    if (!confirm('האם למחוק את קובץ ה-PDF המצורף לכנס?')) return;

    this.apiService.deleteAttachment(id).subscribe({
      next: () => {
        this.existingAttachmentFileName = null;
        this.conferenceForm.get('attachmentDescription')?.setValue('');
      },
  ...
  saveConference() {
    ...
    const payload = {
      ...
      Links: formValue.links,
      // ⭐ חדש: מאפשר לעדכן את התיאור המילולי גם בלי להעלות קובץ חדש
      AttachmentDescription: formValue.attachmentDescription
    };
    ...
    action.subscribe({
      next: (res: any) => {
        // ⭐ חדש: מזהה הכנס לשימוש בהעלאת הקובץ - במצב עריכה כבר יש לנו אותו,
        // במצב יצירה חדשה צריך לקחת אותו מתשובת השרת
        const savedId = this.isEditMode
          ? formValue.id
          : (res?._id || res?.Id || res?.id);

        if (this.selectedFile && savedId) {
          this.apiService.uploadAttachment(savedId, this.selectedFile, formValue.attachmentDescription).subscribe({
            next: () => this.finishSave(),
            error: (err) => {
              console.error('Error uploading attachment:', err);
              alert('הכנס נשמר, אך העלאת הקובץ המצורף נכשלה. ניתן לנסות שוב מתוך מסך העריכה.');
              this.finishSave();
            }
          });
        } else {
  ...
  // ⭐ חדש: הלוגיקה המשותפת שהייתה בעבר בתוך ה-subscribe הראשי -
  // הופרדה כדי לרוץ גם אחרי שמירת הכנס וגם (במידת הצורך) אחרי העלאת הקובץ
