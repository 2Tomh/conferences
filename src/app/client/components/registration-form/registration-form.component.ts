...
//           allowsPoster: conf.AllowsPoster || conf.allowsPoster || false,
//           posterGuidelines: conf.PosterGuidelines || conf.posterGuidelines || '',
//           // ⭐ שומרים את השדה (אות גדולה, תואם ל-C# JSON serialization) כדי שהפילטר למטה יוכל להשתמש בו
//           isExternalOnly: conf.IsExternalOnly === true
//         }))
//           // ⭐ עודכן: מסננים החוצה גם לפי השדה מה-DB, בנוסף לרשימה הקשיחה
//           .filter(conf =>
//             !conf.isExternalOnly &&
//             !this.EXCLUDED_CONFERENCE_NAMES.some(excluded => excluded.toLowerCase() === (conf.name || '').toLowerCase())
//           );
...
export class RegistrationFormComponent implements OnInit, OnChanges {
  ...
  private pendingRegistrationData: any = null;
  ...
  get registrationAmountForCurrency(): number {
  ...
  onSubmit() {
    ...
    // עוברים לשלב 3 (בחירת מטבע) - currentStep הוא מה שקובע איזה פאנל מוצג
    this.currentStep = 3;
  }
  ...
  confirmCurrencyAndPay(): void {
    if (!this.pendingRegistrationData) return;
    this.paymentInputData = {
      ...this.pendingRegistrationData,
      amount: this.registrationAmountForCurrency,
      currency: this.selectedCurrency
    };
    // ⭐ תוקן: זה היה חסר - בלי זה currentStep נשאר 3 לנצח, וה-HTML (שכבר
    // בודק currentStep === 4 ולא showCurrencyStep) לא מציג שום דבר חדש
    this.currentStep = 4;
  }
  get f() { return this.regForm.controls; }
  get af() { return this.abstractForm.controls; }
