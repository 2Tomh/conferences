...
export class ConferenceEventsComponent implements OnInit {
  ...
  private readonly CATEGORY_CLASS_MAP: { [key: string]: string } = {
    'humanities and arts': 'cat-humanities-and-arts',
  ...
  // בודקת קודם את השדה מה-DB (IsExternalOnly, באות גדולה כי
  ...
  getCategoryClass(conf: any): string {
    const singleCategory = (conf.Category || conf.category || '').toLowerCase();
    if (singleCategory && this.CATEGORY_CLASS_MAP[singleCategory]) {
      return this.CATEGORY_CLASS_MAP[singleCategory];
    }

    const categoriesArray: string[] = conf.categories || conf.Categories || [];
    for (const cat of categoriesArray) {
      const normalized = (cat || '').toLowerCase();
      if (this.CATEGORY_CLASS_MAP[normalized]) {
        return this.CATEGORY_CLASS_MAP[normalized];
      }
    }
  ...
  // ⭐ חדש: פורמט תאריך קריא לכרטיסיות ("Oct 20, 2026") - אם התאריך לא
  // תקין/חסר, מחזיר את המחרוזת הגולמית (או ריק) במקום לקרוס
